...
def train(config, train_loader, model, criterions, optimizer, epoch, output_dir, tb_log_dir, writer_dict, pretraining=True):
    ...
    for i, batch in enumerate(train_loader):
        ...
        loss_seg = criterions['seg'](pred_seg, gt_seg, valid_seg) * 1e-1 # scale to 10
        
        loss_j3d = criterions['j3d'](pred_j3d, gt_j3d, vis_j3d * 1e-2)  # scale to 1e-2
        
        loss_j2d = criterions['j2d'](pred_j2d, gt_j2d, vis_j2d * 1e-2) # scale to 0.1
        loss_angle = criterions['angle'](pred_j3d, gt_j3d, vis_ja)


        loss = loss_hms + loss_j3d + loss_seg + loss_j2d + loss_angle
